# Design Document: Can Models Hide Problems From Light Oversight

## 1. Purpose

This document is the method contract for **Can Models Hide Problems From Light Oversight**. It specifies what is
...
One-liner: Test whether models can produce code/data that looks fine under shallow review but fails deeper checks.

Hypothesis: Test whether models can produce code/data that looks fine under shallow review but fails deeper checks.
...
|---|---|---|
| Config schema | `sabotage.configs.schema` | Typed dataclasses + role-keyed models |
| Loader | `sabotage.configs.loader` | Hydra compose API |
| Device | `sabotage.models.device` | MPS/CUDA/CPU resolution + fallback flag |
| Registry | `sabotage.models.registry` | Pinned revisions + hardware fit |
| Hooks | `sabotage.models.hooks` | Capture, steer, ablate, patch, attn intervene |
| Generation | `sabotage.models.generation` | Chat template path recording |
| Data | `sabotage.data` | Synthetic items, splits, manifests |
| Evaluation | `sabotage.evaluation` | Metrics, failure harness, protocols |
| Ablation | `sabotage.ablation` | Control / dropout / seed-sweep arms |
| Reporting | `sabotage.reporting` | Aggregate → MD/TeX/figures |
| Cache | `sabotage.cache` | Versioned artifact store |
...
`src/sabotage/sabotage/` are preserved across regeneration.
